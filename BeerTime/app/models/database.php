...
class Database
{
  ...
  // Fetch all rows.
  public static function all($table, $limit = 1000) {
    $query = self::connectionInit()->prepare("SELECT * FROM `".$table."` ORDER BY `id` ASC LIMIT ".$limit."");
    $query->execute();
  ...
  public static function update($table, $id, $datas) {
    $columns = [];
    foreach ($datas as $k => $v) { array_push($columns, "{$k} = :{$k}"); }
