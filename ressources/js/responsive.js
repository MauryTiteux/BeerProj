...
function sticky(){
  ...
  if (scrollTop > (height - navheight)) {
    ...
    $('#cssmenu ul').removeClass('nav_animate');
    setTimeout(function(){
      $('#logo').css({
        'left':-190, 
        'transition':'.5s'
...
//Navigational Menu
$('#cssmenu ul li a').click(function(a){
  var menuPlace = $(this).index();
  $('html, body').animate({
    scrollTop : $('section').eq(menuPlace).offset().top - $('#cssmenu ul').height()
  }, 700);
});
